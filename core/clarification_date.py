...
from constants import general, stage_8
...
def clarification(instance):
    ...
    return (
        general.DATE_CLARIFICATION.format(date=output),
        buttons_yes_or_not(),
...
async def handle_date_confirmation(user_answer, instance):
    if user_answer == "да":
        data_from_db = await queries.get_user_data(instance.user_id)
        format_data_from_db.format(instance, data_from_db)
        return (
            stage_8.ASK_QUANTITY.format(position=instance.data_list[0]),
            ReplyKeyboardRemove(),
            Main.waiting_for_quantity_sold,
        )
    elif user_answer == "нет":
        return (
            general.INPUT_CORRECT_DATE,
            ReplyKeyboardRemove(),
            CheckDate.waiting_for_correct_date,
        )
    else:
        return (general.INPUT_YES_NO, buttons_yes_or_not(), None)
...
def ask_correct_date(user_answer, instance):
    try:
        ...
        instance.date = date
        return (
            general.DATE_CLARIFICATION.format(date=date),
            buttons_yes_or_not(),
            CheckDate.waiting_for_confirm_date,
        )
    except ValueError:
        return (
            f"{general.INCORRECT_INPUT}\n{general.INPUT_CORRECT_FORMAT}",
            ReplyKeyboardRemove(),
            None,
        )
