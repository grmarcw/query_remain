...
def test_get_element_for_change(
        instance_stage_one,
        mock_render_list,
    ...
    result_for_cancel_stage_one = change_data.get_element_for_change(
        'отменить', instance_stage_one)
    result_for_diff_element = change_data.get_element_for_change(
        'элемент не из списка', instance_stage_one)
    result = change_data.get_element_for_change(
        'молоко', instance_stage_one)
    result_for_cancel_stage_two = change_data.get_element_for_change(
        'отменить', instance_stage_two)

    assert result_for_cancel_stage_one == (t.LIST_TEXT,
                                 t.BUTTONS_YES_NOT,
                                 FillingStates.waiting_for_data_confirmation)
    assert result_for_diff_element == (t.DONT_EXIST,
                                       t.BUTTON_GENERATOR,
                                       None)
    assert result == (constants.INPUT_NEW_NAME,
                      t.DELETE_KB,
                      ChangingData.change)
    assert result_for_cancel_stage_two == (t.RENDER_DICT,
                                 t.BUTTONS_YES_NOT,
                                 FillingStates.waiting_for_data_confirmation)


def test_change_data(instance_stage_one,
                               mock_render_list,
                               mock_buttons_yes_not,
                               instance_stage_two,
                               mock_render_dict,
    ...
    result_for_cancel_stage_two = change_data.change_data('отменить', instance_stage_two)
    result_stage_two = change_data.change_data('капуч', instance_stage_two)
    ...
    assert result_for_cancel_stage_two == (t.RENDER_DICT,
                                 t.BUTTONS_YES_NOT,
                                 FillingStates.waiting_for_data_confirmation)
    assert result_stage_two == (t.RENDER_DICT,
                      t.BUTTONS_YES_NOT,
                      FillingStates.waiting_for_data_confirmation)
    assert instance_stage_two.compositions == {'молоко': ['капуч'], 'зерно': ['американо']}


def test_get_composition_for_change(instance_stage_two,
                                    mock_render_dict,
                                    mock_not_exist,
                                    mock_buttons_yes_not,
                                    mock_delete_kb,
    ...
    result = change_data.get_composition_for_change(
        "молоко", instance_stage_two
    )
    result_for_cancel_stage_three = change_data.get_composition_for_change(
        "отменить", instance_stage_three
    )
    result_for_incorrect_position_stage_three = change_data.get_composition_for_change(
        "сливки", instance_stage_three
    )
    result_stage_three = change_data.get_composition_for_change(
    ...
    assert result_for_cancel_stage_three == (t.CHECKING_CORRECTNESS,
                                             t.BUTTONS_YES_NOT,
                                             FillingStates.waiting_for_data_confirmation)
    assert result_for_incorrect_position_stage_three == (t.DONT_EXIST,
                                                         t.BUTTON_GENERATOR,
                                                         None)
    assert result_stage_three == (t.QUANTITY,
                      t.DELETE_KB,
